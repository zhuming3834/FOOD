...
//**************************************************************************************//
//                      Chakery 2015年09月22日20:28:12
//**************************************************************************************//
-(NSMutableArray *)select:(NSString *)tableName withWhere:(NSDictionary *)dic page:(NSString *)page pageSize:(NSString *)pageSize orderBy:(NSString *)orderby desc:(BOOL)desc{
    NSMutableArray *array = [[NSMutableArray alloc]init];
    NSMutableString *sql = [[NSMutableString alloc]init];
    [sql appendFormat:@"SELECT * FROM %@", tableName];
    //如果有条件
    ...
    if (page.length > 0 && pageSize.length > 0) {
        [sql appendFormat:@" LIMIT %@ OFFSET %@", pageSize, page];
    }
    FMResultSet *result = [_dataBase executeQuery:sql withParameterDictionary:dic];
    while (result.next) {
        NSMutableDictionary *dict = [[NSMutableDictionary alloc]init];
